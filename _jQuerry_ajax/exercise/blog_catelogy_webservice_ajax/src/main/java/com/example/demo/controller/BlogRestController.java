...
//   Xem chi tiết một bài viết
    ...
    //khong co view nen phai tra ve ResponseEntity
    public ResponseEntity<Page<Blog>> getBlogPage( @PageableDefault(value = 1,sort = "id",direction = Sort.Direction.ASC) Pageable pageable){
        Page<Blog> blogListPage = blogService.findAll(pageable);
        if (blogListPage.isEmpty()){
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity<>(blogListPage, HttpStatus.OK);
    }
